...
/* 14. Display all the departments that have a minimum salary greater 
than that of department 1 (HAVING Subquery). */
...
/* 15. List names of employees whose department is listed in the department table 
(WHERE Subquery). */
...
/* 18. What is the average of the total salary paid to employees in each department 
(FROM Subquery). */

SELECT AVG(DeptTotSalary) as AvgDeptSalary
 FROM
	(
	SELECT SUM(Salary) as DeptTotSalary, DeptID
	FROM dbo.Employee
	GROUP BY DeptID
	) DeptSalary;
